...
custom_layout = {
        'num_obj': 7,
        'obj_class_name': ["bird", "bird", "bush", "bird", "grass", "sky-other", "tree"],
        'obj_bbox': [
            [0.5838750004768372, 0.5268749594688416, 0.8354843854904175, 0.7720416188240051],
            [0.3061875104904175, 0.4309374988079071, 0.5134062170982361, 0.6391666531562805],
            [0.5, 0.0, 1.0, 0.4583333432674408],
            [0.45457810163497925, 0.4977499842643738, 0.6076875329017639, 0.7242916822433472],
            [0.0, 0.39791667461395264, 1.0, 1.0],
            [0.0, 0.0, 0.9781249761581421, 0.4208333194255829],
            [0.0, 0.29374998807907104, 0.42656248807907104, 0.44999998807907104]
        ]
    }
...
def main():
    ...
    if cfg.sample.save_cropped_images:
        dir_names.extend(['generated_cropped_imgs'])
    ...
    init_layout = {
        ...
        'obj_class': torch.LongTensor(layout_length).fill_(data_loader.dataset.vocab['object_name_to_idx']['__null__']),
        'is_valid_obj': torch.zeros([layout_length]),
        'obj_class_name': ['__null__' for _ in range(layout_length)]
    }

    # The first object will be the special __image__ object
    init_layout['obj_bbox'][0] = torch.FloatTensor([0, 0, 1, 1])
    init_layout['obj_class'][0] = data_loader.dataset.vocab['object_name_to_idx']['__image__']
    init_layout['is_valid_obj'][0] = 1.0
    init_layout['obj_class_name'][0] = '__image__'


    custom_layout['obj_class'] = [data_loader.dataset.vocab['object_name_to_idx'][obj_class_name] for obj_class_name in custom_layout['obj_class_name']]

    print('custom_layout', custom_layout)
    meta_data = {
        'num_obj': [],
        'obj_class_name': [],
        'obj_bbox': [],
    ...
    for obj_id in range(custom_layout['num_obj']):

        for key, value in init_layout.items():
            if obj_id == 0:
                meta_data[key].append(get_clone(value))
            else:
                meta_data[key].append(get_clone(meta_data[key][-1]))

        meta_data['num_obj'].append(obj_id + 1)
        meta_data['obj_bbox'][-1][obj_id + 1] = torch.FloatTensor(custom_layout['obj_bbox'][obj_id])
        meta_data['obj_class'][-1][obj_id + 1] = custom_layout['obj_class'][obj_id]
        meta_data['obj_class_name'][-1][obj_id + 1] = custom_layout['obj_class_name'][obj_id]
        meta_data['is_valid_obj'][-1][obj_id + 1] = 1.0

    for key in ['obj_bbox', 'obj_class', 'is_valid_obj']:
        meta_data[key] = torch.stack(meta_data[key], dim=0)
        print(key, meta_data[key].shape, meta_data[key])

    model_kwargs = {
        'obj_class': meta_data['obj_class'].to(dist_util.dev()),
        'obj_bbox': meta_data['obj_bbox'].to(dist_util.dev()),

        'is_valid_obj': meta_data['is_valid_obj'].to(dist_util.dev())
    }
    ...
    for sample_idx in range(cfg.sample.sample_times):
        if cfg.sample.sample_method == 'dpm_solver':
            ...
            x_T = th.randn((custom_layout['num_obj'], 3, cfg.data.parameters.image_size, cfg.data.parameters.image_size), device=dist_util.dev())
            ...
            all_results = sample_fn(
                model_fn, (custom_layout['num_obj'], 3, cfg.data.parameters.image_size, cfg.data.parameters.image_size),
        ...
        for img_idx in range(custom_layout['num_obj']):
            black_image = (torch.ones(3, cfg.data.parameters.image_size, cfg.data.parameters.image_size) * 0.0).cpu()
            filename = 'custom_image_{}'.format(img_idx)
            obj_num = meta_data['num_obj'][img_idx]
            obj_class = meta_data['obj_class'][img_idx]
            obj_class_name = meta_data['obj_class_name'][img_idx]
            is_valid_obj = meta_data['is_valid_obj'].long().tolist()[img_idx]
            obj_bbox = meta_data['obj_bbox'][img_idx]
            absolute_obj_bbox = obj_bbox.clone()
            absolute_obj_bbox[:, 0::2] = obj_bbox[:, 0::2] * cfg.data.parameters.image_size
            absolute_obj_bbox[:, 1::2] = obj_bbox[:, 1::2] * cfg.data.parameters.image_size
            ...
            if cfg.sample.save_cropped_images:
                ...
                for obj_idx in range(obj_num):
                    saved_cropped_image_name = "{}_{}_{}_{}_{}.png".format(filename, obj_idx, obj_class[1:1 + obj_num][obj_idx], obj_class_name[1:1 + obj_num][obj_idx].replace(' ', '-'), sample_idx)
                    utils.save_image(
            ...
            if cfg.sample.save_images_with_bboxs:
                ...
                generated_images_with_bboxs = utils.draw_bounding_boxes(
                    ...
                    labels=[meta_data['obj_class_name'][img_idx][i] for i, j in enumerate(is_valid_obj) if j]
                )
                imageio_save_image(
